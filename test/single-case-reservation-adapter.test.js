...
test("reserva válida",async()=>assert.deepEqual(await adapter({reservation_key:`case-import:${ID}`,case_number:NUMBER,status:"reserved"}).verify(ID,NUMBER),{verified:true,caseImportId:ID,caseNumber:NUMBER,evidenceId:`case-import:${ID}`}))
test("preflight PVR expõe apenas estado de adoção sem alterar a prova canônica",async()=>assert.deepEqual(await adapter({reservation_key:`case-import:${ID}`,case_number:NUMBER,status:"reserved"}).verifyPvrAdoption(ID,NUMBER),{reservationKey:`case-import:${ID}`,caseNumber:NUMBER,status:"reserved"}))
