...
function memoryPool(rows = [], checkpoint = null) {
  const state = { rows: structuredClone(rows), checkpoint: structuredClone(checkpoint), queries: [], writes: 0 }
  return { state, async query(sql, params) {
    ...
    if (text.includes("JOIN single_case_apply_checkpoints")) { const selected=state.checkpoint&&state.checkpoint.case_import_id===params[0]?state.rows.filter(row=>params[1].includes(row.authorization_id)).map(row=>({...row,checkpoint_authorization_ids:state.checkpoint.authorization_ids,checkpoint_authorization_consumed_by:state.checkpoint.authorization_consumed_by})):[];return{rowCount:selected.length,rows:structuredClone(selected)} }
...
test("retry carrega somente o par consumido vinculado ao checkpoint",async()=>{const rows=signedRows(),provenance="executor:lease-original";rows.forEach(row=>{row.consumed_at=NOW;row.consumed_by=provenance});const ids=rows.map(row=>row.authorization_id),pool=memoryPool(rows,{case_import_id:plan().caseImportId,authorization_ids:ids,authorization_consumed_by:provenance}),repository=createSingleCaseAuthorizationRepository({pool}),records=await repository.loadForCheckpoint({...expected(plan()),authorizationIds:ids});assert.equal(records.length,2);assert(records.every(record=>record.consumedBy===provenance))})
test("origem consumidora divergente da proveniência do checkpoint é rejeitada",async()=>{const rows=signedRows(),ids=rows.map(row=>row.authorization_id);rows.forEach(row=>{row.consumed_at=NOW;row.consumed_by="executor:lease-other"});const pool=memoryPool(rows,{case_import_id:plan().caseImportId,authorization_ids:ids,authorization_consumed_by:"executor:lease-original"}),repository=createSingleCaseAuthorizationRepository({pool});await assert.rejects(()=>repository.loadForCheckpoint({...expected(plan()),authorizationIds:ids}),/AUTHORIZATION_CONSUMPTION_PROVENANCE_MISMATCH/)})
test("caminho inicial continua excluindo autorizações já consumidas",async()=>{const rows=signedRows();rows.forEach(row=>{row.consumed_at=NOW;row.consumed_by="executor:lease-original"});assert.equal((await load(rows)).records.length,0)})
...
test("CLI real falha fechado sem configuraÃ§Ã£o", async () => assert.rejects(()=>cli.main({argv:["--case-import-id",plan().caseImportId],env:{}}),/POSTGRES_MODE_REQUIRED/))
