...
;(async () => {
  ...
  // ============================================================
  // REPRODUÇÃO: criarPastaCliente is not a function
  // ============================================================
  ...
  try {
    ...
    const result = await flow.executeLiveCaseFlow(deps.u)
  ...
  try {
    ...
    const deps = {
      ...
      hsCriarNota: async () => {},
      hsCriarNotaNegocio: async () => {},
      // Nenhum fallback legado injetado
      checkpointRepository: {
        async load(hash) { return checkpointStore.get(hash) || null },
        async save(hash, checkpoint) { checkpointStore.set(hash, checkpoint) }
      }
  ...
  // 5. Ausência de dependência obrigatória interrompe antes de escritas posteriores
  //    (contact e deal são criados antes do passo drive — isso é esperado pelo design)
  //    O que importa: o passo drive falha SEM fallback automático
  try {
    const checkpointStore = new Map()
    const writeAttempts = []
    const deps = {
      ...
      // criarPastaCliente deliberadamente AUSENTE
      montarPropsContatoHubSpot: (phone, u) => ({ phone }),
      montarPropsAusentesContatoHubSpot: (existing, props) => ({}),
      montarTituloNegocioHubSpot: (u, opts) => "Deal",
      getHubSpotDealStateProps: (u) => ({}),
    ...
    // O passo drive falha — uploadDrive (documentos) não deve ser chamado após a falha
