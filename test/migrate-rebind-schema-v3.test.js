...
const { main, inspectSchema, validatePreconditions, applyMigration, parseMode, MIGRATION_ID, TABLE_NAME, COLUMN_NAME } = require("../scripts/migrate-rebind-schema-v3")
  ...
  const client = {
    query: async (sql, params) => {
      ...
      if (sql.includes("ALTER TABLE") && sql.includes("ADD CONSTRAINT")) {
        state.schema.reasonCheckDefinition = "CHECK ((reason = ANY (ARRAY['CONTACT_RECONCILED_AFTER_DIVERGENCE'::text, 'PLAN_REGENERATED_AFTER_SAFE_CORRECTION'::text, 'AUTHORIZATION_PAIR_REFRESHED_AFTER_EXPIRY'::text])))"
        state.schema.reasonAcceptsAuthRefresh = true
        state.mutationCount++
        return { rowCount: 0, rows: [] }
      }
      return { rowCount: 0, rows: [] }
...
test("parseMode aceita --dry-run", () => {
  assert.equal(parseMode(["--dry-run"]), "dry_run")
})

test("parseMode aceita --apply", () => {
  assert.equal(parseMode(["--apply"]), "apply")
})

test("parseMode aceita ausência de argumentos", () => {
  assert.equal(parseMode([]), "dry_run")
})

test("parseMode rejeita modo desconhecido", () => {
  assert.throws(() => parseMode(["--invalid"]), /INVALID_MIGRATION_MODE/)
})

test("parseMode rejeita modos duplicados", () => {
  assert.throws(() => parseMode(["--apply", "--apply"]), /INVALID_MIGRATION_MODE/)
})

test("parseMode rejeita --dry-run e --apply juntos", () => {
  assert.throws(() => parseMode(["--dry-run", "--apply"]), /INVALID_MIGRATION_MODE/)
})

test("parseMode rejeita múltiplos argumentos desconhecidos", () => {
  assert.throws(() => parseMode(["--unknown", "extra"]), /INVALID_MIGRATION_MODE/)
})

test("dry_run detecta migração necessária", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: false, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = 'CONTACT_RECONCILED_AFTER_DIVERGENCE'::text))", reasonAcceptsContact: true, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
...
test("apply adiciona motivo de renovação", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: false, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = 'CONTACT_RECONCILED_AFTER_DIVERGENCE'::text))", reasonAcceptsContact: true, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client, state } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
  ...
  assert.equal(result.afterSchema.reasonAcceptsContact, true)
  assert.equal(result.afterSchema.reasonAcceptsPlan, true)
  assert.equal(result.afterSchema.reasonAcceptsAuthRefresh, true)
  assert.equal(state.mutationCount, 2)
})

test("apply é idempotente quando schema já está atualizado", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: true, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = ANY (ARRAY['CONTACT_RECONCILED_AFTER_DIVERGENCE'::text, 'PLAN_REGENERATED_AFTER_SAFE_CORRECTION'::text, 'AUTHORIZATION_PAIR_REFRESHED_AFTER_EXPIRY'::text])))", reasonAcceptsContact: true, reasonAcceptsPlan: true, reasonAcceptsAuthRefresh: true, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
  const result = await main({ argv: ["--apply"], PoolClass, output: () => {} })
  assert.equal(result.mode, "apply")
  assert.equal(result.beforeSchema.reasonAcceptsAuthRefresh, true)
  assert.equal(result.afterSchema.reasonAcceptsAuthRefresh, true)
})

test("tabela ausente bloqueia", async () => {
  const schema = { tableExists: false, columnExists: false, columnType: null, columnNullable: false, reasonCheckName: null, reasonCheckDefinition: null, reasonAcceptsContact: false, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
...
test("coluna ausente bloqueia", async () => {
  const schema = { tableExists: true, columnExists: false, columnType: null, columnNullable: false, reasonCheckName: null, reasonCheckDefinition: null, reasonAcceptsContact: false, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
...
test("múltiplos CHECKs ambíguos bloqueiam", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: false, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = 'CONTACT_RECONCILED_AFTER_DIVERGENCE'::text))", reasonAcceptsContact: true, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, extraCheck: { name: "reason_check_2", definition: "CHECK ((reason = 'OTHER'::text))" }, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
...
test("falha após primeira alteração provoca rollback", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: false, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = 'CONTACT_RECONCILED_AFTER_DIVERGENCE'::text))", reasonAcceptsContact: true, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false, applyFails: true }
  const { client } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
...
test("nenhum índice ou trigger alterado", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: false, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = 'CONTACT_RECONCILED_AFTER_DIVERGENCE'::text))", reasonAcceptsContact: true, reasonAcceptsPlan: false, reasonAcceptsAuthRefresh: false, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false, indexes: [{ indexname: "single_case_rebind_audit_case_source_current_idx", indexdef: "CREATE UNIQUE INDEX ..." }] }
  const { client, state } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
  const result = await main({ argv: ["--apply"], PoolClass, output: () => {} })
...
test("apply idempotente quando constraint já tem os três motivos exatos", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: true, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = ANY (ARRAY['CONTACT_RECONCILED_AFTER_DIVERGENCE'::text, 'PLAN_REGENERATED_AFTER_SAFE_CORRECTION'::text, 'AUTHORIZATION_PAIR_REFRESHED_AFTER_EXPIRY'::text])))", reasonAcceptsContact: true, reasonAcceptsPlan: true, reasonAcceptsAuthRefresh: true, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client, state } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
...
test("apply bloqueia se constraint permissiva demais com motivo desconhecido", async () => {
  const schema = { tableExists: true, columnExists: true, columnType: "text", columnNullable: true, reasonCheckName: "single_case_rebind_audit_reason_check", reasonCheckDefinition: "CHECK ((reason = ANY (ARRAY['CONTACT_RECONCILED_AFTER_DIVERGENCE'::text, 'PLAN_REGENERATED_AFTER_SAFE_CORRECTION'::text, 'AUTHORIZATION_PAIR_REFRESHED_AFTER_EXPIRY'::text, 'UNKNOWN_REASON'::text])))", reasonAcceptsContact: true, reasonAcceptsPlan: true, reasonAcceptsAuthRefresh: true, unexpectedConstraints: [], blockingTriggerFound: false, blockingIndexFound: false }
  const { client } = createMockClient(schema)
  const PoolClass = MockPoolClass
  PoolClass.prototype.client = client
  await assert.rejects(main({ argv: ["--apply"], PoolClass, output: () => {} }), /UNKNOWN_REASONS_PRESENT/)
})
