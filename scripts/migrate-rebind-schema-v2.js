...
const MIGRATION_ID = "single-case-apply-rebind-audit-v2"
const COLUMN_NAME = "reconciliation_evidence_hash"
const EXPECTED_REASONS = Object.freeze(["CONTACT_RECONCILED_AFTER_DIVERGENCE", "PLAN_REGENERATED_AFTER_SAFE_CORRECTION"])
...
async function validatePreconditions(client) {
  ...
  if (schema.columnNullable && reasonAcceptsPlan) {
    return { schema, alreadyMigrated: true, reasonCheckName }
  }
  if (schema.columnNullable) throw new Error("COLUMN_ALREADY_NULLABLE")
...
async function applyMigration(client, reasonCheckName) {
  await client.query(`ALTER TABLE ${TABLE_NAME} ALTER COLUMN ${COLUMN_NAME} DROP NOT NULL`)

  await client.query(
    `ALTER TABLE ${TABLE_NAME} DROP CONSTRAINT ${reasonCheckName}`
  )
  await client.query(
    `ALTER TABLE ${TABLE_NAME} ADD CONSTRAINT ${reasonCheckName} CHECK (reason = ANY (ARRAY['CONTACT_RECONCILED_AFTER_DIVERGENCE'::text, 'PLAN_REGENERATED_AFTER_SAFE_CORRECTION'::text]))`
  )

  const afterSchema = await inspectSchema(client)
  if (!afterSchema.columnNullable) throw new Error("MIGRATION_COLUMN_NOT_NULLABLE")
  ...
  try {
    if (mode === "dry_run") {
      ...
      try {
        ...
        output(JSON.stringify({ mode: "dry_run", schema }, null, 2))
        return { mode: "dry_run", schema }
      } catch (error) {
