...
async function renderPdfPages(buffer, maxPages, limits = DEFAULT_LIMITS, dependencies = {}) {
  ...
  try {
    ...
    return { pages, totalPages: document.numPages, truncated: document.numPages > maxPages }
  } finally {
...
function canonicalizePipeline(pipeline, file, pageNumber) {
  ...
  return {
    ...
    benefitNumbers, processNumbers, benefitTypes, birthDates,
...
function consolidateCase({ sourceFolder, importId, files, analyzed, ignored, hashes, relativeRoot }) {
  ...
  return {
    ...
    numerosProcessoEncontrados: processNumbers, tiposBeneficioEncontrados: benefitTypes,
    datasNascimentoEncontradas: birthDates,
