...
function legacyVaultPage(nonce = "") {
...
function vaultPage(nonce = "") {
  const safeNonce = text(nonce, 100)
  return `<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Dados e credenciais do cliente</title>
<style>
body{font-family:system-ui;background:#f5f3ef;color:#252019;margin:0}.box{max-width:680px;margin:5vh auto;background:#fff;padding:24px;border-radius:16px;box-shadow:0 8px 30px #0002}
h1{font-size:1.4rem;margin:0 0 20px}h2{font-size:1.05rem;margin:0;padding:14px 0 8px;color:#604b20}.section{margin:0 0 18px;border:1px solid #e5e0d7;border-radius:12px;padding:0 14px 8px}
.row{padding:10px 0;border-bottom:1px solid #eee}.row:last-child{border-bottom:0}.label{color:#71685e;font-size:.8rem}.value-line{display:flex;align-items:center;gap:8px}.value{font-size:1rem;overflow-wrap:anywhere;flex:1}
input{box-sizing:border-box;width:100%;padding:12px;margin:8px 0;border:1px solid #bbb;border-radius:8px}button{padding:11px 16px;border:0;border-radius:8px;background:#8b6b22;color:#fff;font-weight:700;margin:4px 4px 4px 0}
.copy-field{padding:7px 10px;margin:0;background:#eee;color:#3d372f;font-size:1rem}.secondary{background:#666}.hidden{display:none}.error{color:#a40000}.status{min-height:24px;color:#315f31}
</style></head><body><main class="box"><h1>🔐 Dados e credenciais do cliente</h1>
<section id="login"><p>Digite a senha mestre administrativa para abrir este registro.</p><input id="master" type="password" autocomplete="current-password"><button id="unlock">Desbloquear</button><p id="error" class="error"></p></section>
<section id="record" class="hidden"><div id="profile"></div><section class="section"><h2>🔑 Credenciais Gov.br / Meu INSS</h2><label class="label" for="password">Senha</label><input id="password" type="password" autocomplete="off"><button id="reveal" class="secondary">Mostrar</button><button id="copy">📋 Copiar</button><button id="save">Salvar senha</button></section><p id="status" class="status"></p></section>
</main><script nonce="${safeNonce}">
const base=location.pathname;let csrf="";const $=id=>document.getElementById(id);const esc=value=>String(value??"").replace(/[&<>\"]/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[char]));
const field=(label,value)=>({label,value:String(value||"Não informado")});
function renderProfile(profile){
 const address=[profile.street,profile.addressNumber,profile.addressComplement,profile.district].filter(Boolean).join(", ");
 const groups=[
  ["🪪 Identificação",[field("Caso",profile.caseNumber),field("Nome",profile.name),field("CPF",profile.cpf),field("RG",profile.rg),field("Nascimento",profile.birthDate),field("Nome da mãe",profile.motherName),field("Nome do pai",profile.fatherName),field("Filiação",profile.parentage)]],
  ["📞 Contato",[field("Telefone",profile.phone),field("E-mail",profile.email)]],
  ["🏠 Endereço",[field("Endereço",address),field("Cidade/UF",[profile.city,profile.state].filter(Boolean).join(" - ")),field("CEP",profile.postalCode),field("Referência",profile.addressReference)]],
  ["💼 Dados profissionais",[field("Estado civil",profile.maritalStatus),field("Profissão",profile.profession),field("NIT/PIS",profile.nitPis)]],
  ["📁 Arquivos",[field("Pasta no Drive",profile.driveUrl)]]
 ];
 const values=[];
 $("profile").innerHTML=groups.map(group=>{const rows=group[1].map(item=>{const index=values.push(item)-1;return '<div class="row"><div class="label">'+esc(item.label)+'</div><div class="value-line"><div class="value">'+esc(item.value)+'</div><button class="copy-field" data-copy="'+index+'" type="button" title="Copiar '+esc(item.label)+'" aria-label="Copiar '+esc(item.label)+'">📋</button></div></div>'}).join("");return '<section class="section"><h2>'+esc(group[0])+'</h2>'+rows+'</section>'}).join("");
 document.querySelectorAll("[data-copy]").forEach(button=>button.onclick=async()=>{const item=values[Number(button.dataset.copy)];await navigator.clipboard.writeText(item.value);$("status").textContent=item.label+" copiado."});
}
async function load(){const response=await fetch(base+"/dados",{credentials:"same-origin"});if(!response.ok)return;const data=await response.json();csrf=data.csrf;$("login").classList.add("hidden");$("record").classList.remove("hidden");renderProfile(data.profile||{});$("password").value=data.password||"";}
$("unlock").onclick=async()=>{const response=await fetch(base+"/entrar",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({masterPassword:$("master").value})});$("master").value="";if(!response.ok){$("error").textContent=response.status===429?"Muitas tentativas. Aguarde alguns minutos.":"Senha incorreta.";return}await load()};
$("reveal").onclick=()=>{$("password").type=$("password").type==="password"?"text":"password"};$("copy").onclick=async()=>{await navigator.clipboard.writeText($("password").value);$("status").textContent="Senha copiada."};
$("save").onclick=async()=>{const response=await fetch(base+"/senha",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json","x-csrf-token":csrf},body:JSON.stringify({password:$("password").value})});$("status").textContent=response.ok?"Senha atualizada com segurança.":"Não foi possível salvar."};load();
</script></body></html>`
}
