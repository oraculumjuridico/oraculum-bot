...
function createDriveSingleCaseAdapter({ client, rootFolderId, timeoutMs = 30000 } = {}) {
  ...
  return Object.freeze({
    findAreaFolders(destination) { if (!validDestination(destination)) fail("DRIVE_DESTINATION_INVALID"); return list({ kind: "folder", parentId: rootFolderId, logicalId: destination.logicalId, name: destination.name }) },
    ...
    findCaseFolders(parentId, destination) { if (!validId(parentId) || !validDestination(destination)) fail("DRIVE_DESTINATION_INVALID"); return list({ kind: "folder", parentId, logicalId: destination.logicalId, name: destination.name }) },
